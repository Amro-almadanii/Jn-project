...
const CampaignItem = ({ campaign }) => {
  ...
  return (<div className={classes.campaignItem}>
    ...
    <div className={classes.box}>
        ...
        <div className={classes.cardItems}>
          ...
          <p> {
            campaignDetail.leads.map((lead) => (
              <Link key={lead.id} className={classes.leadLink}
                    to={`/marketing/leads/lead-detail/${lead.id}`}> {lead.type} </Link>))} </p>
        </div>
      ...
      {!editLeadCampaign && <div className={classes.cardBox}>
        <Card className={classes.cardChildren}>
          <h2> Events:</h2>
          {campaignDetail.events.map((event) => (<Card className={classes.cardChildren} key={event.id}>
        ...
        <Card className={classes.cardChildren}>
          <h2> TVs :</h2>
          {campaignDetail.tvs.map((tv) => (<Card className={classes.cardChildren} key={tv.id}>
      ...
      {editLeadCampaign && <UpdateLeadsOfCampaign leadsOfCampaign={campaignDetail.leads}/>}
    </div>
  </div>);
};
